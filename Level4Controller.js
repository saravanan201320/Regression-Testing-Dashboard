...
Level4PageControllers.controller('Level4PageController', function($scope,$http,$filter,$rootScope,$location){
  ...
  if(typeof($rootScope.selectedLevel3Data) == "string"){  	
    $rootScope.selectedLevel3Data = JSON.parse($rootScope.selectedLevel3Data);
  }  
  ...
  var selectedLevel3Header = "";
  var selectedLevel3Header =  $rootScope.selectedLevel3Data.title;
  $rootScope.selectedLevel3Header = selectedLevel3Header;
  
  $scope.level4Data =[]
  $scope.level4PageData = [];
  for (var i = 0; i < jsonData.length; i++){    
    if(jsonData[i][dataHeader[0]] == $rootScope.selectedHeader && jsonData[i][dataHeader[1]] == $rootScope.selectedLevel2Header && jsonData[i][dataHeader[2]] == selectedLevel3Header){
      $scope.level4Data.push(jsonData[i][dataHeader[3]]);               
    }
  }
  ...
  $scope.chartObject3 = {};
  $scope.chartObject3.type = "TreeMap";
  var d = [];
   d.push(['level2', 'level1', 'Count', 'RAG'], [selectedLevel3Header, null, 0, 20]);
   for (var i = 0; i < $scope.level4PageData.length; i++) {
    d.push($scope.level4PageData[i]);
   };
   $scope.chartObject3.data = d;
...
//     $rootScope.selectedLevel4Data = $scope.chartObject3.data[col + 1][0];
